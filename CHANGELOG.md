...
### Added
- **OAuth browser SSO is back** (was removed in the v1.x → v2.0 ODBC→ADBC rewrite). Implemented entirely in M against GizmoSQL Enterprise's server-side OAuth: `StartLogin` calls `GET /oauth/initiate` and opens `auth_url` in Power BI's embedded browser; after the IdP redirects to the server's `/oauth/callback`, `FinishLogin` polls `GET /oauth/token/{session_uuid}` for the IdP's ID token. That token is presented to the stock Flight SQL driver through the Flight **handshake** (`username = "token"`, JWT as password — the same convention JDBC and gizmosql-ui use), so the server verifies it via JWKS and issues its own session JWT — no custom driver needed. (Presenting it instead as `adbc.flight.sql.authorization_header` fails with "invalid token issuer", since that path only accepts the server's own tokens.) Assumes the default `--oauth-port` `31339`; the OAuth endpoint's TLS certificate must be trusted by the client (`Web.Contents` cannot skip verification). Re-authentication is required when the JWT expires (the flow has no refresh token).
...
- `dept`/`emp` primary-key/foreign-key fixture in CI and `tests/Relationships.query.pq`, guarding navigation and primary-key emission over a PK/FK schema.
- *Relationships* section in `README.md`. Unlike the v1.x ODBC connector, the ADBC navigator does **not** auto-create foreign-key relationships — automatic FK import is a built-in of the native `Odbc.DataSource` handler (it reads `SQLForeignKeys`), and Power Query exposes no API for a hand-rolled navigator to declare relationships. (The one documented workaround, Matt Masson's `Table.NestedJoin` "implicit" sample, was spiked and confirmed in Power BI Desktop to not survive the `SqlView.Generator` layer; DirectQuery folding was unaffected.) The connector declares each table's primary key, so Power BI's built-in **Autodetect** finds relationships reliably on same-named keys (e.g. `emp.dept_id` → `dept.dept_id`).
