...
### Relationships (not auto-detected — by design)
The ADBC navigator does **not** surface foreign-key relationships to the Power BI model, unlike the v1.x ODBC connector. Auto FK import is a built-in of the native `Odbc.DataSource`/`Adbc.DataSource` handlers (`SQLForeignKeys` / ADBC GetObjects); Power Query has no API for a hand-rolled navigator to declare FKs. The only documented workaround (Matt Masson's deleted `Table.NestedJoin` "implicit" sample) was spiked (PR #4) and **does not work** here — its relationship metadata doesn't survive the `SqlView.Generator` layer because our tables load as independent queries rather than statically-joined `#table`s. Verified in Power BI Desktop: relationships did not auto-detect (folding was unaffected). Resolution: users rely on Power BI's built-in **Autodetect** (works on same-named keys + PK, which the connector declares). Do not re-attempt the nested-join approach without a fundamentally different navigator architecture. The `dept`/`emp` PK+FK CI fixture and `tests/Relationships.query.pq` guard the primary-key emission this depends on.
